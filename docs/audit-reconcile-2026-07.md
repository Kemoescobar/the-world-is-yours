...
| Dispersion encore visible sur Chantier LIVE | **OPEN** | Gate OK ; banner si Ère **active** + quêtes sans `ere_objectif_id` (données actuelles → presque toujours). Affiner le seuil ou lier les quêtes aux objectifs. |
...
| Ravitaillement (remplir quêtes depuis roadmap) | **NEXT** | Design : [`ravitaillement.md`](./ravitaillement.md) — code non commencé |
