...
import { createInstrumentalSchema, patchInstrumentalSchema } from '../schemas.js';
...
router.patch('/:id', requireAuth, validateBody(patchInstrumentalSchema), async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    if (error.code === 'PGRST116') return res.status(404).json({ error: 'instru introuvable' });
